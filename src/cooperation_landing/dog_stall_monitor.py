...
class DogStallMonitor:
    ...
    def __init__(self):
        ...
        self.min_translation_m = max(0.0, float(rospy.get_param('~min_translation_m', 0.01)))
        self.min_yaw_rad = max(0.0, float(rospy.get_param('~min_yaw_rad', 0.02)))
        self.linear_cmd_threshold = max(0.0, float(rospy.get_param('~linear_cmd_threshold', 0.02)))
        self.angular_cmd_threshold = max(
            0.0, float(rospy.get_param('~angular_cmd_threshold', 0.05))
        )
        self.cmd_timeout_s = max(self.window_s, float(rospy.get_param('~cmd_timeout_s', 1.5)))
        self.pose_timeout_s = max(0.1, float(rospy.get_param('~pose_timeout_s', 1.0)))
        self.required_windows = max(1, int(rospy.get_param('~required_windows', 2)))
        self.recovery_cooldown_s = max(0.0, float(rospy.get_param('~recovery_cooldown_s', 2.0)))
        ...
        self._timer = rospy.Timer(rospy.Duration(1.0 / check_rate_hz), self._check_stall)
    ...
    def _check_stall(self, _event):
        ...
        with self._lock:
            ...
            if self._cmd_time is None or (now - self._cmd_time).to_sec() > self.cmd_timeout_s:
                self._clear_window()
                return
            ...
            if (
                ...
                and (now - self._last_recovery_time).to_sec() < self.recovery_cooldown_s
            ):
            ...
            translation_stalled = translation_expected and translation < self.min_translation_m
    ...
    def _call_stand(self):
        try:
            rospy.wait_for_service(self.stand_service, timeout=self.stand_service_timeout_s)
